...
graphHourlyInput <- function(id, state_list, county_list){
  ...
  fluidRow(
    fluidRow(
      column(6,plotOutput(nameSpace("AQIHourlyPlot")))
    ),
    ...
    fluidRow(
      box(
        ...
        selectInput(nameSpace("startTime"), "Select a start time", hour_list, selected = 0),
        selectInput(nameSpace("stopTime"), "Select a stop time", hour_list, selected = 0)
      )
